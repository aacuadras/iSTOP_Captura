...
def tablaCliente(cl, value):
    ...
    def save_client():
        ...
        if(info1.get() != "" and info2.get() != "" and info4.get() != "" and info5.get() != "" and info6.get() != ""):
            SQL = "UPDATE HISTORIA SET guia='" + str(info5.get()) + "', PROVEEDOR='" + str(info6.get()) + "' WHERE FECHA1='" + str(info1.get()) + "' AND PLACAS='" + str(info2.get()) + "' AND observacion='" + str(info4.get()) + "';"
            cur.execute(SQL)
            cur.commit()
...
def create_cancel():
    def cancel_folio():
        SQL = "UPDATE HISTORIA SET PROVEEDOR='CANCELADO', FOLIO=00, embarque='CANCELADO', observacion='CANCELADO', PLACAS='CANCELADO', CLIENTE=3 WHERE guia='" + str(entry4.get()) + "';"
        cur.execute(SQL)
        cur.commit()
